...
MCWebReq::MCWebReq()
{
 ...
 std::string MCWebReq::parseResData( cocos2d::network::HttpResponseEx *response )
 {
	 if(!(response->isSucceed())){
		 ...
		 log("error:%s, code:%d", error, response->getResponseCode());
		 log("XQ5WebReq::getParseResData error = %s",error);
		 return "";
	 }
	 ...
	 log("XQ5WebReq::getParseResData Sucess = %s",str.c_str());
	 return str;
 }
