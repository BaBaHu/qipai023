...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions
{
    for (SKPaymentTransaction *transaction in transactions) {
        ...
        std::string str_receipt;
        if (NULL != receipt) {
            str_receipt = receipt.UTF8String;
            NSLog(@"==ios str_receipt lenth:%lu", str_receipt.size());
        }
        ...
        switch (transaction.transactionState) {
            ...
            case SKPaymentTransactionStatePurchased:
                ...
                _iosiap->delegate->onPaymentEvent(identifier, str_receipt, event, transaction.payment.quantity);
                NSLog(@"-----交易完成 --------");
                break;
        ...
        //_iosiap->delegate->onPaymentEvent(str_receipt, event, transaction.payment.quantity);
