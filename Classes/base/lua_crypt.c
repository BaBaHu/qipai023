...
des_key(lua_State *L, uint32_t SK[32]) {
	size_t keysz = 0;
	const unsigned char * key = (unsigned char*)luaL_checklstring(L, 1, &keysz);
	if (keysz != 8) {
...
*/
static void
rc4_crypt(struct rc4_state *const state, const uint8_t *inbuf, uint8_t *outbuf, int buflen)
{
	int i;
	uint8_t j;
...
static int
lrc4_crypt(lua_State *L) {
	size_t keysz = 0;
	const uint8_t * key = (const uint8_t *)luaL_checklstring(L, 1, &keysz);
	if (keysz == 0) {
		luaL_error(L, "Invalid key size %d!!!", (int)keysz);
	}

	struct rc4_state cipher;
	rc4_init(&cipher, key, (int)keysz);

	size_t textsz = 0;
	const uint8_t *text = (const uint8_t *)luaL_checklstring(L, 2, &textsz);
	if (textsz == 0) {
		luaL_error(L, "Invalid key size %d!!!", (int)textsz);
	}
	...
	rc4_crypt(&cipher, (unsigned char*)text, buffer, (int)textsz);

	lua_pushlstring(L, (const char *)buffer, textsz);
	return 1;
}
...
// The biggest 63bit prime
//#define P 0xffffffffffffffc5ull

// The biggest 32bit prime
// #define P 0xfffffffbull

// The
#define P 0x7FFFFFFFFFFFFFE7ull
...
static const luaL_Reg crypt_functions[] = {
	{ "hashkey", lhashkey },
...
#endif
	srandom(time(NULL));
	luaL_register(L, "crypt", crypt_functions);
	return 1;
}