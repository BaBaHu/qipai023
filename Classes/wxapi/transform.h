...
*/
#ifndef __CODE_TRANSFORM_H__
#define __CODE_TRANSFORM_H__
...
namespace transform
{
    namespace private_
    {
		...
		template <>         struct utf_type<1> { typedef unsigned char  type_t; };
		template <>         struct utf_type<2> { typedef unsigned short type_t; };
		template <>         struct utf_type<4> { typedef unsigned int   type_t; };
 
        template <typename T, size_t X>
        struct check
        {
            static const bool value = ((sizeof(T) == sizeof(typename utf_type<X>::type_t)) && !is_pointer<T>::value);
		};
	}
