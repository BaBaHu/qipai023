...
void  CServerListData::addCustomServer(const tagGameServer& server)
{
	CGameServerItemMap::iterator it = m_CustomServers.find(server.wServerID);
	if (it != m_CustomServers.end())
		return;
	...
	//设置数据
	memcpy(&pGameServerItem->m_GameServer, &server, sizeof(tagGameServer));

	//寻找父项
	if (server.wNodeID != 0)
	{
		pGameServerItem->m_pGameKindItem = SearchGameKind(server.wKindID);
		pGameServerItem->m_pParentListItem = SearchGameNode(server.wNodeID);
	}
	else
	{
		pGameServerItem->m_pGameKindItem = SearchGameKind(server.wKindID);
		pGameServerItem->m_pParentListItem = SearchGameKind(server.wKindID);
	}

	//设置索引
	m_CustomServers[server.wServerID] = pGameServerItem;
}

CGameServerItem* CServerListData::searchCustomServer(word wServerID)
{
	CGameServerItemMap::iterator it = m_CustomServers.find(wServerID);
	if (it == m_CustomServers.end())
		return 0;
...
CGameServerItem* CServerListData::searchCustomServerByUUID(dword dwUUID)
{
	//word wServerID = ServerIDByUUID(dwUUID);

	word wServerID = dwUUID;

	return searchCustomServer(wServerID);
}

void  CServerListData::addMyCustomTable(const tagGameServer& server)
{
	CGameServerItemMap::iterator it = m_MyCustomTables.find(server.wServerID);
	if (it != m_MyCustomTables.end())
		return;
	...
	//设置索引
	m_MyCustomTables[server.wServerID] = pGameServerItem;
}

void  CServerListData::rmvMyAllCustomTables()
{
	CGameServerItemMap::iterator it = m_MyCustomTables.begin();
	for (; it != m_MyCustomTables.end(); it++)
	{
		CGameServerItem * pGameServerItem = it->second;
		//删除数据
		SafeDelete(pGameServerItem);
	}
	m_MyCustomTables.clear();
}

 CGameServerItem* CServerListData::searchMyCustomTable(word wKindID)
{
	CGameServerItemMap::iterator it = m_MyCustomTables.begin();
	for (; it != m_MyCustomTables.end(); it++)
	{
		if (it->second->m_GameServer.wKindID == wKindID)
		{
			return it->second;
		}
	}
