...
protected:
	...
	CGameServerItemMap				m_CustomServers;					//自定义房间索引
	CGameServerItemMap				m_MyCustomTables;
	//内核变量
protected:
...
public:
	//自定义房间接口
	void addCustomServer(const tagGameServer& server);
	CGameServerItem* searchCustomServer(word wServerID);
	CGameServerItem* searchCustomServerByUUID(dword dwUUID);
	void addMyCustomTable(const tagGameServer& server);
	void rmvMyAllCustomTables();
	CGameServerItem* searchMyCustomTable(word wKindID);
};
