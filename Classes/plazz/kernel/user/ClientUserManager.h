...
protected:
	...
	tagCustomTableInfo				m_CustomTableInfo;					//自定义桌子
	//扩展属性
protected:
...
public:
	//重要等级
	virtual unsigned int GetImportOrder();
	//用户信息
	...
	//自定义桌子信息
	virtual tagCustomTableInfo * GetCustomTableInfo() { return &m_CustomTableInfo; }
...
public:
	...
	//用户URL
	virtual const char* GetIconUrl() { return m_UserInfo.szIconUrl; }
	//用户类型
	virtual int GetUserType() { return m_UserInfo.cbUserType; }
	//经验魅力
public:
	//用户奖牌
	...
	//魅力数值
	virtual uint32 GetUserLoveLiness() { return m_UserInfo.lLoveLiness; }
...
protected:
	...
	ClientUserItemVector		m_UserItemStorage;					//存储数组
...
public:
	...
	//更新自定义房间信息
	virtual bool UpdateUserCustomTableInfo(IClientUserItem * pIClientUserItem, const tagCustomTableInfo& customTableInfo);
...
class CGameUserManager
{
	//用户变量
protected:
	ClientUserItemVector			m_UserItemLookon;					//旁观列表
	ClientUserItemVector			m_UserItemStorage;					//存储列表
...
public:
	...
	virtual bool DeleteUserItem(IClientUserItem * pIClientUserItem);
	//增加用户
	virtual IClientUserItem * ActiveUserItem(const tagUserInfo & UserInfo, const tagCustomFaceInfo & CustomFaceInfo, byte cbCompanion, const char* pszUserNote);
