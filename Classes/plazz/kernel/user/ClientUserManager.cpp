...
CClientUserItem::CClientUserItem()
{
	...
	zeromemory(&m_CustomTableInfo, sizeof(m_CustomTableInfo));
...
//重要等级
unsigned int CClientUserItem::GetImportOrder()
{
	//构造等级
	int nOrder=0;
...
{
	uint32 lPlayCount=GetUserPlayCount();
...
{
	MELOG(MELOG_INFO, "flow->CPlazaUserManager::DeleteUserItem ....\n");
	//查找用户
...
bool CPlazaUserManager::UpdateUserItemScore(IClientUserItem * pIClientUserItem, const tagUserScore * pUserScore)
{
	...
	MELOG(MELOG_INFO, "flow->CPlazaUserManager::UpdateUserItemScore ....\n");
	//获取用户
...
bool CPlazaUserManager::UpdateUserItemStatus(IClientUserItem * pIClientUserItem, const tagUserStatus * pUserStatus)
{
	...
	//获取用户
	tagUserInfo * pUserInfo=pIClientUserItem->GetUserInfo();
	MELOG(MELOG_INFO, "flow->CPlazaUserManager::UpdateUserItemStatus  %d, %d \n", pUserInfo->wTableID, pUserInfo->wChairID);
...
//更新自定义房间信息
bool CPlazaUserManager::UpdateUserCustomTableInfo(IClientUserItem * pIClientUserItem, const tagCustomTableInfo& customTableInfo)
{
	//效验参数
	ASSERT(pIClientUserItem != 0);
	...
	//memcpy(&CustomTableInfo, pCustomTableInfo, sizeof(tagCustomTableInfo));

	//设置数据
	for (word i = 0; i < MAX_CUSTOMTABLE_CHAIR; ++i)
		pCustomTableInfo->lChairScore[i] = customTableInfo.lChairScore[i];
	pCustomTableInfo->cbCurRound = customTableInfo.cbCurRound;
	if (customTableInfo.cbExData>0)
	{
		pCustomTableInfo->cbExData = customTableInfo.cbExData;
		pCustomTableInfo->dwCreateFlag = customTableInfo.dwCreateFlag;
		pCustomTableInfo->dwCreateUser = customTableInfo.dwCreateUser;
		pCustomTableInfo->cbMaxRound = customTableInfo.cbMaxRound;
		pCustomTableInfo->wChairCount = customTableInfo.wChairCount;
		pCustomTableInfo->wTableID = customTableInfo.wTableID;
	}

	//通知更新
	ASSERT(m_pIUserManagerSink != 0);
	if (m_pIUserManagerSink)
		m_pIUserManagerSink->OnUserCustomTableUpdate(pIClientUserItem);
...
//更新自定义房间信息
bool CGameUserManager::UpdateUserCustomTableInfo(IClientUserItem * pIClientUserItem, const tagCustomTableInfo& customTableInfo)
{
	//效验参数
	ASSERT(pIClientUserItem != 0);
