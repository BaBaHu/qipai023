...
struct tagUserInfo
{
	...
	dword							lExpand;
	dword							dwExpand;
	byte							cbMobileUser;                       //改动过

	//附加信息
	char							szIconUrl[256];						//头像地址
	byte							cbUserType;							//登录方式
};
...
struct tagUserInfoHead
{
	...
	// 今日局数信息 [10/28/2012 Care]
	dword							dwDayWinCount;						//胜利盘数
	dword							dwDayLostCount;						//失败盘数
	dword							dwDayDrawCount;						//和局盘数
	dword							dwDayFleeCount;						//逃跑盘数
...
#define MAX_CUSTOMTABLE_CHAIR 8
#define SUB_S_CUSTTINFO			255									//更新自定义桌子信息
struct tagCustomTableInfo
{
	byte							cbExData;	//附加数据标识
	word							wTableID;
	word							wChairCount;		// 椅子数量
	uint32							lChairScore[MAX_CUSTOMTABLE_CHAIR];	// 房间用户分数
	dword							dwCreateUser;	// 创建用户?
	dword							dwCreateFlag;	// 创建的 Flag?
	byte							cbCurRound;		// 当前圈数
	byte							cbMaxRound;		// 最大圈数
};
...
struct tagRankItem
{
	...
	char							szNickname[64];	//LEN_NICKNAME*2	//用户昵称
};
