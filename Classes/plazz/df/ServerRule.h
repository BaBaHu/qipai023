...
//快速加入
#define SR_FAST_ENTRY               0x01000000							//快速加入
#define SR_ACTIVITYTIPS				0x02000000							//活动提示

#define SR_USER_INVISIBLE			0x80000000							//对用户不可见，只能通过id进入 改动过

#define RULE_3_1_1						0x00000001L				
#define RULE_3_1_2						0x00000002L				
#define RULE_3_1_3						0x00000004L				
#define RULE_2_1						0x00000008L				
#define RULE_2_2						0x00000010L				
#define RULE_4_1_1						0x00000020L				
#define RULE_4_1_2						0x00000040L				
#define RULE_4_1_3						0x00000080L			
#define RULE_4_1_4				        0x00000100L  
...
public:
	...
	//快速加入
	static bool IsFastEntry(dword dwServerRule) { return (dwServerRule&SR_FAST_ENTRY) != 0; }
	//活动提示
	static bool IsActivityTips(dword dwServerRule) { return (dwServerRule&SR_ACTIVITYTIPS) != 0; }
	//用户不可见 改动过
	static bool IsUserInvisible(dword dwServerRule) { return (dwServerRule&SR_USER_INVISIBLE) != 0; }
	//聊天规则
public:
	//禁止公聊
...
public:
	...
	//快速加入
	static void SetFastEntry(dword & dwServerRule, bool bEnable) { (bEnable == true) ? dwServerRule |= SR_FAST_ENTRY : dwServerRule &= ~SR_FAST_ENTRY; }
	//活动提示
	static void SetActivityTips(dword & dwServerRule, bool bEnable) { (bEnable == true) ? dwServerRule |= SR_ACTIVITYTIPS : dwServerRule &= ~SR_ACTIVITYTIPS; }
};
