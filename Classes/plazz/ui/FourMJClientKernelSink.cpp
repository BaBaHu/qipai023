...
//构造函数
FourMJClientKernelSink::FourMJClientKernelSink()
{
...
//旁观消息
bool FourMJClientKernelSink::OnEventLookonMode(void* data, int dataSize)
{
	return true;
}

//场景消息
bool FourMJClientKernelSink::OnEventSceneMessage(byte cbGameStatus, bool bLookonUser, void* data, int dataSize)
{
	switch (cbGameStatus)
	{
	case GS_MJ_FREE:
	...
	case GS_MJ_PLAY:
		{
			...
			for (size_t i = 0; i < DF::shared()->GetGamePlayer(); i++)
			{
				...
				for (size_t j = 0; j < pCsc->cbWeaveCount[i]; j++)
				{
					...
					dict61["cbCombinationCard"] = LuaValue::intValue((int)pCsc->WeaveItemArray[i][j].cbCombinationCard);
...
//游戏消息
bool FourMJClientKernelSink::OnEventGameMessage(int sub, void* data, int wSize)
{
	if (!m_bRunning)
		return true;
...
bool FourMJClientKernelSink::OnGameStart(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameSelectPai(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameChuPai(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameSendPai(void* data, word wSize)
{
	...
	LuaValueDict dict3;
	for (size_t i = 0; i < MAX_GANGCARD; i++)
	{
		dict3[StringUtils::format("%d", i + 1).c_str()] = LuaValue::intValue((int)pCsc->cbGangCardData[i]);
	}
	dict["cbGangCardData"] = LuaValue::dictValue(dict3);
...
bool FourMJClientKernelSink::OnGameOperateNotify(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameOperateResult(void* data, word wSize)
{
	...
	dict["cbCombinationCard"] = LuaValue::intValue((int)pCsc->cbCombinationCard);
...
bool FourMJClientKernelSink::OnGameChiHu(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameGangScore(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameOver(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameTrustee(void* data, word wSize)
{
...
bool FourMJClientKernelSink::OnGameChangeCard(void* data, word wSize)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
//设置托管
void FourMJClientKernelSink::setTrustee(bool Trustee)
{
...
//出牌消息
void FourMJClientKernelSink::chuPai(int idxPai)
{
...
//出牌操作消息
void FourMJClientKernelSink::operatePai(int nOperateCode, int idxPai)
{
...
//选牌定缺消息
void FourMJClientKernelSink::selectPai(int nType)
{
...
//换三张消息
void FourMJClientKernelSink::changePai(int nCard1, int nCard2, int nCard3)
{
