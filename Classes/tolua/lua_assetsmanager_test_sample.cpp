...
static int lua_cocos2dx_AssetsManager_setAppRunPath(lua_State* L)
{
    if (nullptr == L)
        return 0;
    ...
    if (0 == argc)
    {
        ...
        std::string pathToSave = FileUtils::getInstance()->getWritablePath();
        pathToSave += "setup";
        CCLOG("set app run path : %s", pathToSave.c_str() );
#if (CC_TARGET_PLATFORM != CC_PLATFORM_WIN32)
        DIR *pDir = NULL;
    ...
    CCLOG("'setAppRunPath' function wrong number of arguments: %d, was expecting %d\n", argc, 0);
    return 0;
}
...
int register_assetsmanager_test_sample(lua_State* L)
{
    ...
    tolua_function(L, "setAppRunPath", lua_cocos2dx_AssetsManager_setAppRunPath);
    tolua_endmodule(L);
    return 0;
}
